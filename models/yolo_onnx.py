            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        ...
                        # y_kpt_split=[]
                        # conf_kpt_split = []
                        # for i in range(self.nkpt*3):
                        #     if (i%3==0):
                        #         x_kpt_split.append(kpt_split[i])
                        #     if (i%3==1):
                        #         y_kpt_split.append(kpt_split[i])
                        #     if (i%3==2):
                        #         conf_kpt_split.append(kpt_split[i])
                        # print("--------", (torch.cat(x_kpt_split, -1).size()))
                        # x_kpt_split = (x_kpt_split * 2. - 0.5 + kpt_grid_x.repeat((1,1,1,1,self.nkpt))) * self.stride[i] 
                        # y_kpt_split = (y_kpt_split * 2. - 0.5 + kpt_grid_y.repeat((1,1,1,1,self.nkpt))) * self.stride[i] 
                        # conf_kpt_split = conf_kpt_split.sigmod()
                        # #kpt = (x_kpt * 2. - 0.5 + self.grid[i].repeat((1,1,1,1,self.nkpt))) * self.stride[i]  # xy
                        # kpt = torch.cat([x_kpt_split, y_kpt_split, conf_kpt_split], dim=-1)
